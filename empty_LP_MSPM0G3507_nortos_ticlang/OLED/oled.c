...
unsigned char B=0x01<<4;//0001 0000
 unsigned char GRAM[1024*8];
//测试函数
//检测OLED模块和程序是否正确
//尝试刷新显示
...
void yanshi()
{
    unsigned int i;
    for(i=0;i<64;i++){
        OLED_ShowString(80, 0, "x1=", 12);
        OLED_ShowString(80, 10, "y1=", 12);
        OLED_ShowNum(110, 0, i, 2,12);
        OLED_ShowNum(110, 10, i, 2,12);
        OLED_DrawPoint(i,i);
        OLED_Refresh();
    }
}
...
void OLED_ClearPoint(unsigned int x,unsigned int y){
    ...
    GRAM[n*128+x]=GRAM[n*128+x]&~(0x01<<n);
}
...
void yanshi1()
{
    // OLED_ShowString(0, 0, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", 16);
    // OLED_ShowString(0, 4, "Yi Qi Wan OLED Ping Mu", 16);
    
    
    
    OLED_ShowChinese(80, 50, 2,16);
}
